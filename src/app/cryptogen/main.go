...
func generate() {
	...
	tlsCADir := filepath.Join(*outputDir, "tlsca")
	var err error
	rootCA, err = ca.NewCA(caDir, "rootech", "fabric-ca-server")
	if err != nil {
		fmt.Printf("Error generating signCA for org %s:\n%v\n", "rootech", err)
		os.Exit(1)
	}
	// generate TLS CA
	tlsCA, err = ca.NewCA(tlsCADir, "tlsrootech", "tls-fabric-ca")
	if err != nil {
...
func generatePeerOrg(baseDir string, orgSpec OrgSpec) {
	...
	*/
	err := msp.GenerateVerifyingMSP(mspDir, rootCA, tlsCA)
	if err != nil {
		fmt.Printf("Error generating MSP for org %s:\n%v\n", orgName, err)
		os.Exit(1)
	}

	generateNodes(peersDir, orgSpec.Specs, rootCA, tlsCA)
	...
	generateNodes(usersDir, users, rootCA, tlsCA)
...
func generateOrdererOrg(baseDir string, orgSpec OrgSpec) {
	...
	*/
	...
	generateNodes(orderersDir, orgSpec.Specs, rootCA, tlsCA)
