...
func (dbclient *CouchDatabase) CreateDatabaseIfNotExist() (*DBOperationResponse, error) {
	...
	if dbInfo == nil && couchDBReturn.StatusCode == 404 {
		...
		if dbResponse.Ok == true {
			log.Printf("Created database %s ", dbclient.DBName)
		}
...
func (dbclient *CouchDatabase) GetDatabaseInfo() (*DBInfo, *DBReturn, error) {
	...
	if err == nil {
		log.Printf("GetDatabaseInfo() dbResponseJSON: %s\n", dbResponseJSON)
	}
...
func (couchInstance *CouchInstance) VerifyCouchConfig() (*ConnectionInfo, *DBReturn, error) {
	...
	dbResponseJSON, err := json.Marshal(dbResponse)
	if err == nil {
		log.Printf("VerifyConnection() dbResponseJSON: %s\n", dbResponseJSON)
	}
	...
	if err != nil {
		log.Printf("Unable to connect to CouchDB,  error: %s   Check the admin username and password.\n", err.Error())
...
func (dbclient *CouchDatabase) EnsureFullCommit() (*DBOperationResponse, error) {
	...
	if err != nil {
		log.Printf("Failed to invoke _ensure_full_commit Error: %s\n", err.Error())
		return nil, err
	}
	defer closeResponseBody(resp)
	...
	if dbResponse.Ok == true {
		log.Printf("_ensure_full_commit database %s \n", dbclient.DBName)
	}
...
func (dbclient *CouchDatabase) ReadDoc(id string) (*CouchDoc, string, error) {
	...
	if err != nil {
		if couchDBReturn != nil && couchDBReturn.StatusCode == 404 {
			log.Println("Document not found (404), returning nil value instead of 404 error")
	...
	if strings.HasPrefix(mediaType, "multipart/") {
		...
		for {
			...
			log.Printf("part header=%s", p.Header)
			...
			default:
				...
				if strings.TrimSpace(contentDispositionParts[0]) == "attachment" {
					...
					case "gzip": //See if the part is gzip encoded
						...
						log.Println("Retrieved attachment data")
...
func (dbclient *CouchDatabase) ReadDocRange(startKey, endKey string, limit, skip int) (*[]QueryResult, error) {
	...
	log.Printf("HTTP Response: %s", bytes.Replace(dump, []byte{0x0d, 0x0a}, []byte{0x20, 0x7c, 0x20}, -1))
	//handle as JSON document
	...
	log.Printf("Total Rows: %d", jsonResponse.TotalRows)
	...
	for _, row := range jsonResponse.Rows {
		...
		if jsonDoc.Attachments != nil {
			...
			log.Printf("Adding JSON document and attachments for id: %s", jsonDoc.ID)
...
func (couchInstance *CouchInstance) handleRequest(method, connectURL string, data []byte, rev string,
	...
	for attempts := 0; attempts < maxRetries; attempts++ {
		...
		_, err := payloadData.ReadFrom(bytes.NewReader(data))
		if err != nil {
			log.Println("payloadata.ReadFrom(), err:", err)
		}
		...
		if errResp != nil {
			...
			log.Printf("Retrying couchdb request in %s. Attempt:%v  Error:%v",
			...
			log.Printf("Retrying couchdb request in %s. Attempt:%v  Couch DB Error:%s,  Status Code:%v  Reason:%v",
	...
	if resp.StatusCode >= 400 {
		log.Println("resp.StatusCode > 400")
		...
		log.Printf("Couch DB Error:%s,  Status Code:%v,  Reason:%s",
